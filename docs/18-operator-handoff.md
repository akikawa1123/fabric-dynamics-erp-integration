...
2. 直近データが必要なら再シード（容量 Active 時）:
   ...
   uv run --no-project --with deltalake --with pyarrow --with azure-identity `
     ...
     --lakehouse-id 2c580bf3-6fde-4416-b082-1dc4cc15a5bb
   ```
...
- 通知本文は `manufacturing-quality-agent-obo` のM365 Copilot直リンクへ更新済み:
...
M365 Copilotで聞く（1発で異常確認＋文書＋判断推奨まで）:
...
- Fabric OBOでCRCA/LOT/ST-07-PRESSの事実、Foundry IQで8D/PFMEA/Control Plan/校正手順
