...
2. 直近データが必要なら再シード（容量 Active 時。**Activator/incident と同じロットに揃える**ため `--lot` を付ける）:
   ```powershell
   ...
   `--lot` は異常ロットを固定する。Activator の UDF パラメータ `lotId`（= incident/Teams 通知のロット）と同じ値にすると、**工場AI（Lakehouse）= incident = 営業** のロット表記が揃いシームレスになる。毎日同じ台本で見せるなら、Activator 側 `lotId` を `LOT-CRCA-DEMO-007` 等の日付なし固定値にして同じ値で seed するのが楽。

> 注: 直近で品質異常が出ない場合、シードの異常帯（最新から6〜25分）が古くなっただけ。上記再シードで MAX が現在時刻に更新され復活する。

**異常発生（Activator を実際に発火させてフルE2Eを起点から見せる場合）**:
Eventhouse の `Telemetry` に固定ロットの異常バーストを注入し、Activator(`torque_alert`, 平均>=50Nm)→UDF→Logic Apps を自然発火させる。接続情報は `rti_info.json` か環境変数（`KUSTO_QUERY_URI`/`KUSTO_INGEST_URI`/`KUSTO_DATABASE`）から解決する（リポジトリにハードコードしない）。
...
正常→（既定130秒待機でActivator状態リセット）→異常100件を注入する。`--no-normal` で異常のみ、`--anomaly`/`--product`/`--station`/`--line` で内容を調整できる。発火後は Lakehouse も同じ `--lot` で再シードして工場AIのロットを揃える。
...
- Teams/M365 Copilot 向けの本番入口は **Prompt Agent `manufacturing-quality-agent-obo`**。デプロイ済み Hosted Agent は Responses Protocol / ローカル fixture 用に残す。
  ...
  - 作成/更新: `cd agent; uv run --extra foundry python scripts/provision_prompt_agent.py --with-kb`（agent 名 `manufacturing-quality-agent-obo`）。既定モデルは **gpt-5.4-mini**（gpt-5.4 より ~5-10s 速く、営業 ~27s で M365 Copilot の応答予算内に収まりやすい。candidate 厳守・プレーンテキスト整形は維持）。別モデルにするときは `--model <deployment>`。別名で新規作成した agent は identity が変わるため、Foundry IQ(KB) を使うなら新 identity へ `Search Index Data Reader` を付与する（未付与だと KB が 403）。
...
- **M365 Copilot 「Text not extracted」/ 応答が出ない（重要・既知）**:
  - 原因はエージェント側ではなく **M365 Copilot 表面のレンダリング制限**。公開エージェントでは **ストリーミング・引用(citations)・HTML が非対応**で、非対応フォーマットに依存すると本文がプレースホルダ（"Text not extracted"）になる（Microsoft 公式回答: https://learn.microsoft.com/en-my/answers/questions/5870104/azure-foundry-agent-responds-text-not-extracted-in ）。Foundry Playground / Responses API では同症状が再現しない。
  - 対策（本リポジトリで適用済み）: 最終応答を**プレーンテキスト**化、**引用は最小限**（system.md で集約・最大3箇所程度）、Fabric 照会は 1 ターン 1 回、候補一覧は最大5件。これで factory / sales とも M365 で表示されることを確認済み。
  - まだ不安定なら、引用をさらに削る／本文を短くする。Hosted Agent は Bot Service 経由でこの制限を受けやすい。
- **Foundry IQ（品質文書）**:
...
> WorkPackage ID は、Teams の工場/営業通知の末尾「**Cowork: この WorkPackage WP-… を起点に…**」の行から拾える（M365 Copilot の AI 応答には WP ID は出さない＝採番/担当はワークフロー側の責務のため）。通知をハブに、AI調査→引き継ぎ→Cowork が一連で辿れる。
