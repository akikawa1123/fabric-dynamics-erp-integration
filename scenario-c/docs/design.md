...
| リソース | 名前 | 備考 |
|---|---|---|
| Eventhouse | `eh_manufacturing_rti` | RTI 基盤 |
| KQL Database | `eh_manufacturing_rti` | `Telemetry` テーブル + ERP external table |
| Eventstream | `es_client_telemetry` | カスタムEP(Event Hub互換)→Eventhouse |
| Real-Time Dashboard | `RTI 製造品質モニタリング` | センサー時系列 + ERP 複合分析タイル |
| ERP Lakehouse(既存) | Dataverse Link to Fabric | ショートカット元 |

> 各リソースの itemId / KQL DB クラスタ URI 等の環境固有値は、スクリプト実行時に生成される
> `rti_info.json` 等（`.gitignore` 済）に保存される。WorkspaceId / ERP Lakehouse ID は
> `config.local.json`（`config.example.json` をコピーして作成）で指定する。
