...
3. **古いテレメトリを掃除（クリーンスタート）**
   - 何度もデモすると Telemetry に古い時系列が溜まる。デモ開始の送信時に `--reset` を付けると、Lakehouse / KQL の Telemetry を**全削除してから**送信できる。
     ```powershell
     ...
     ```
   - Lakehouse 側はテーブルを一旦ドロップし、Eventstream 送信で**約1分で再作成**される（要 Eventstream 稼働）。送信せず掃除だけなら `--reset-only`。
